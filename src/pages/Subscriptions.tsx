...
import { Search, CreditCard, Edit, Trash2, Eye, Plus } from 'lucide-react';
...
export const Subscriptions: React.FC = () => {
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('Êtes-vous sûr de vouloir supprimer cet abonnement ?')) {
            try {
                // This would need to be implemented based on your server structure
    ...
    const stats = {
        ...
        activeSubscriptions: subscriptions.filter(s => s.estActif).length,
        expiredSubscriptions: subscriptions.filter(s => new Date(s.dateFin) < new Date()).length,
        totalRevenue: subscriptions.reduce((sum, s) => sum + Number(s.prixMensuel), 0),
    };
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredSubscriptions.map((subscription, index) => (
                                <tr key={subscription.id} className="hover:bg-gray-50 transition-colors fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`badge ${subscription.estActif ? 'badge--status-actif' : 'badge--status-expire'}`}>
                                            {subscription.estActif ? 'Actif' : 'Inactif'}
                                        </span>
                                    </td>
                    ...
                    <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-medium">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-3 bg-gray-50 rounded-lg">
                                ...
                                <span className={`badge ${selectedSubscription.estActif ? 'badge--status-actif' : 'badge--status-expire'}`}>
